...
def _verify_indexes(conn):
    """迁移完成时立刻验证 4 个业务索引都存在——不允许依赖第二次启动补建。"""
    expected = [
...
def migrate_v102(conn):
    ...
    # Step 6：清除默认假数据（v1.0.1 之前留下 0.92/1000000 等系统默认；v1.0.2 后均为空）
    conn.execute(
        "UPDATE settings SET value='' WHERE key IN ('hkd_cny_rate','account_size_cny')"
    )
    # 若键不存在则插入空字符串（同样是空）
...
def do_migration(conn, db_path):
    ...
    bak_path = os.path.join(bak_dir, f'workbench-pre-v102-{ts}.db')
    try:
    ...
    try:
        ...
        # 2) v1.0.1 → v1.0.2 增量
        migrate_v102(conn)
        # 3) 重建索引（即使 SCHEMA 已建，重建为安全；逐 execute 避免隐式 commit）
...
def init_db(seed=False, db_path=None):
    ...
    - 默认假数据清理：hkd_cny_rate / account_size_cny 不会写入任何非空值。
    """
...
class Handler(BaseHTTPRequestHandler):
    server_version = 'Workbench/1.0.2'
...
def main():
    ap = argparse.ArgumentParser(description='A/H 投研交易工作台 v1.0.2')
