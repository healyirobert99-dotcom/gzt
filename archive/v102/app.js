...
/* ===== 首页：今日工作台 ===== */
/* v1.0.2: 正式空库不再自动 seed 美图/道通，真实用户创建第一只股票后
   不应再看到"当前是示例数据"提示。sampleBanner 改写为显式触发函数
   _showSampleNotice()，仅在 fixture 内显式调用 --seed 时由系统提示用户；
   不再在 renderHome 默认显示。如果用户日后通过 fixture 写入示例数据，
   可调用 showSampleNoticeIfFixturePresent() 显示 */
let _sampleShownAt = null;
function isFixtureSeedPresent() {
  // 名称或代码命中 fixture 示例（美图/道通）
  return S.secs.some(s => /美图|道通/.test(s.name || '') || ['01357','688208'].includes(String(s.code)));
}
function showSampleNoticeIfFixturePresent() {
  if (isFixtureSeedPresent() && !_sampleShownAt) {
    _sampleShownAt = nowHHMMSS();
    return `<div class="notice" data-sample="1">
      <span>检测到 <b>示例数据</b>（美图公司、道通科技）。请在标的详情页将研究结论与交易计划编辑为实际内容，数据结构即为正式用法。</span>
