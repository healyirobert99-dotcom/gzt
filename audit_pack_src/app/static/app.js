...
async function openExecutionModal(id) {
  ...
  openModal('更新动态执行判断 · ' + s.name, `
    <div class="form-hint" style="margin-bottom:10px">
      ...
      行情刷新<b>不会自动</b>改写此处的判断；每次提交将作为一条新记录永久保留（append-only）。
    </div>
    ${fld('判断日期', `<input name="execution_date" type="date" value="${esc(prev.execution_date || today())}" required>`, true)}
    <div class="grid2">
      ${fld('判断时价格（' + esc(s.currency) + '）', `<input name="price_snapshot" type="number" step="any" value="${prePrice}">`)}
      ${fld('当前执行判断', `<select name="execution_view" required>${EXECUTION_VIEWS.map(v => `<option value="${esc(v)}"${(prev.execution_view || '等待技术确认') === v ? ' selected' : ''}>${esc(v)}</option>`).join('')}</select>`, true)}
    </div>
    <div class="grid2">
