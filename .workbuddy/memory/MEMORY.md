...
- **离线断言 670 / 11 套件全绿**：v102 38、v103 60、v105 25、v106 28、v107 127、v108 129、
  v109 83、v110 14、行情集成 14、数据更新按钮 42、标的归档 **110**（105 + 5 条 A/H 关联契约）
...
- **当前同步点 `2a709e8`**（2026-09-15，本地 = 远程），228 个跟踪文件。
...
- 真实浏览器（真实服务 + 真实 Chromium）端到端：
  ...
  0 张卡 / 后退进已归档标的详情页，28 断言）＋ `check_ah_link_archive.py`（归档 × A/H 关联
  的**负向**验证，11 断言）；均在沙箱端口 8805 上可重复跑。
