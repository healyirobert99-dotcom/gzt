...
- **离线断言 673 / 11 套件全绿**：v102 38、v103 60、v105 25、v106 28、v107 127、v108 129、
  v109 83、v110 14、行情集成 14、数据更新按钮 42、标的归档 **113**（105 + 5 条 A/H 契约
  + 3 条可见性开关契约）
...
- **`S.secs` 只用于"渲染哪些卡片"**（= 活跃列表）。一切"按 id 取标的"必须走 `findSec`
  （活跃 → 已归档两级查找）；**禁止再把 `S.secs.find(...)` 当"全部标的"用** ——
  2026-09-15 的两处缺陷（A/H 下拉静默清空、详情页操作入口静默失效）都源于此。
  另：全项目**只有 1 个下拉**（A/H）由标的列表动态生成，其余 5 个都是固定枚举。
...
- **当前同步点 `fe06bf2`**（2026-09-15，本地 = 远程），229 个跟踪文件。
...
- **归档不得连带丢掉 A/H 关联**（2026-09-15）：`S.secs` 不含已归档标的，而编辑框的 A/H 下拉
  只照 `S.secs` 生成 → 关联标的被归档后下拉里没有它，浏览器回落首项「— 无 —」，用户
  **只改行业/备注再保存就会把 `ah_link_id` 静默清空**（`app.js::openBasicModal`）。
  修法：候选里补回被归档的关联项并标「· 已归档」，**且仅在"当前已关联且该关联不在活跃列表"
  时补**（不无条件塞入已归档标的）；标的库的 A/H 显示同一口径。
  锁定于 `test_security_archive.py §D#9`；端到端负向验证
  `.tmp_v108x/verify/check_ah_link_archive.py`（修正前 4 红、修正后 11 绿）。
- **归档是可见性开关，不是只读开关**（2026-09-15）：`findSec` 原先只查 `S.secs`，
  于是从 `#/s/{id}`（书签 / 后退 / 标的库的 A/H 链接）打开已归档标的时，详情抽屉的
  「更新动态执行 / 录入交易 / 查看研究 / ··· 更多」**全部静默失效**（后端并没有任何
  mutation 端点校验 `archived_at`，本就允许编辑）。修法：
  `findSec` 改为**活跃 → 已归档**两级查找（活跃优先）。
  锁定于 `test_security_archive.py §D#10`；边界脚本 §E7（修正前 3 红 + 对照组绿）。
  **口径：`S.secs` 只用于"渲染哪些卡片"，一切"按 id 取标的"都必须走 `findSec`。**
...
- 全套离线一键跑 + **与基线交叉核对**：`python .tmp_v108x/verify/run_all_tests.py`
  （本项目铁律：改动前后断言数必须对得上、净变化要有来源）。
- 真实浏览器（真实服务 + 真实 Chromium）端到端：
  `browser_e2e_archive.py`（38 断言）＋ `browser_e2e_archive_edge.py`（**边界**：全归档到
  0 张卡 / 后退进已归档标的详情页 / 已归档详情页操作入口不静默失效，32 断言）＋
  `check_ah_link_archive.py`（归档 × A/H 关联的**负向**验证，11 断言）；
  均在沙箱端口 8805 上可重复跑。
  **agent-browser 的硬约束先读 `ENV-TRAPS.md` ⑪⑩⑦⑤④，别先怀疑产品** —— 2026-09-15
  这些坑各伪装过一次"功能坏掉了"（`get count` 批次合并制造 4 条假红；弹窗未关导致点击
  落空、报 "Element not found"）。**两次假红全是"装置"问题，产品一次都没坏。**
...
- **命令面板（Ctrl+K）搜不到已归档标的**（2026-09-15）：这是"归档 = 从工作台消失"的直接
  后果（下拉只从 `S.secs` 生成）。两条路待用户拍板：① 保持现状（改用首页底部「已归档」区）
  ② 把已归档标的纳入搜索并标注「已归档」。**未擅自改。**
