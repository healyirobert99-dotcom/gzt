...
- **当前同步点 `30b32a7`**（2026-09-15，本地 = 远程），222 个跟踪文件。
...
- 真实浏览器（真实服务 + 真实 Chromium）端到端见 `.tmp_v108x/verify/browser_e2e_archive.py`
  （38 断言）+ `browser_e2e_archive_edge.py`（**边界**：全归档到 0 张卡 / 后退进已归档标的
  详情页，28 断言；均可在沙箱端口 8805 上重复跑）。**agent-browser 的硬约束先读
  `ENV-TRAPS.md` ⑪⑩⑦⑤④，别先怀疑产品** —— 2026-09-15 这些坑各伪装过一次
  "功能坏掉了"，其中 `get count` 的**批次合并**陷阱（⑪）直接制造了 4 条假红、
  而文本证据全 PASS。
