...
`test_v107.py` 在 v1.0.7 为 **126 断言**，本版为 **127 断言**，全部通过。

其中 **5 条被同步改写**（断言对象本身发生契约变更，不是新增规则），
其余 **121 条业务语义断言逐字未动**：

| 断言 | v1.0.7 期望 | v1.0.8 期望 | 变更原因 |
|---|---|---|---|
| §K#7 | 勾选后 `change_confirmed=True`（读导入 JSON 的 `status_change_confirmed`） | 拆为 §K#7（粘贴 JSON 自带 `confirmed=true` → preview 仍为 `False`）+ §K#7b（该 commit 仍被拒绝） | 状态确认不再来自导入 JSON |
| §K#8 | `commit_import_full(token)` 成功 | `commit_import_full(token, [0])` 成功 | 确认改为 commit 请求单独提交 |
| §P#1 | `TARGET_SCHEMA_VERSION = '1.0.7'` | `TARGET_SCHEMA_VERSION = '1.0.8'` | 当前版本契约前移 |
| §P#2 | `server_version = 'Workbench/1.0.7'` | `server_version = 'Workbench/1.0.8'` | 当前版本契约前移 |
| §P#6 | `def commit_import_full(token):` | `def commit_import_full(token, confirmed_status_changes=None):` | 函数签名随契约变更 |

> 净变化：126 → 127（改写 5 条、其中 §K#7 拆为 2 条，其余 121 条原样保留）。
...
7. （可选）复现已知缺陷 R-027 / R-028：
   对同一 preview token 并发提交 8 次 commit —— 预期返回 400 且只写入 1 条；
   实测会返回若干 200（重复追加 execution_reviews）与若干 500（database is locked）。
   详见 §十 与审计文档 §11。
...
- **R-027（本轮交付前验证中发现，未修复）**：同一 token 的**并发** commit 可重复写入
  append-only 的 `execution_reviews`。

  - 根因：`_import_cache_take()` 只读取 cache 不消费，token 在 commit 成功后才由
    `_import_cache_invalidate()` 失效，存在 TOCTOU 窗口；而 spec 要求的 4 类漂移检测
    （status / research version / trade_plan version / 原不存在证券已出现）
    **不覆盖"新增 execution"这类 append-only 写入**。
  ...
  - 对照：新建证券 / 改 status / 改 research 或 trade_plan 的场景，漂移检测能拦住，
    实测 6 线程并发仅 1 次成功、库内仅 1 行。
  - 影响面：仅本机单用户工作台的**毫秒级重复提交**；顺序重复提交行为正确（400 且不重写）。
  - 建议 v1.0.9 修复：cache 中加 per-token `in-flight` 标记，`take` 时原子置位，
    失败释放、成功失效。
- **R-028（本轮交付前验证中发现，未修复）**：并发 commit 可能返回 **HTTP 500**
  而非 400。`_mut()` 只把 `sqlite3.IntegrityError` 映射为 400，未捕获
  `sqlite3.OperationalError`（实测同 token 8 并发稳定出现若干次
  `500 服务器错误: database is locked`，本机 2~5 次、随调度波动；不变式：**5xx 恒 > 0**）。
  数据未损坏，仅错误码语义不准。建议 v1.0.9 在 `_mut()` 中一并映射为 400。

> 复现脚本（工作区侧，不随交付）：
> `skills/ah-workbench-release-verify/scripts/concurrency_probe.py`，3/3 次试验稳定判定失败。

> R-027 / R-028 均**未修改任何代码**：本轮 spec 限定"只修复 v1.0.7 审计发现的 9 项、
> 不得扩展任务"，故如实登记，修复需用户明确批准后另开版本。
