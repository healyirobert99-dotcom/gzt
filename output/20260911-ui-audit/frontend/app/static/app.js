...
function uiPositionBand(s) {
  const p = s.plan || {};
  const q = quoteOf(s);
  const price = q && !q.is_stale ? q.current : null;
  const d = decOf(s.currency);
  const zones = [
    ['强赔率区', p.odds_zone_low, p.odds_zone_high, 'odds', zoneStr(p.odds_zone_low, p.odds_zone_high, d)],
    ['加仓区', p.add_zone_low, p.add_zone_high, 'add', zoneStr(p.add_zone_low, p.add_zone_high, d)],
    ['首仓区', p.first_zone_low, p.first_zone_high, 'first', zoneStr(p.first_zone_low, p.first_zone_high, d)],
  ];
  ...
  return `<div class="price-band" aria-label="静态价格位置">
    ${zones.map(z => `<span class="band-segment ${active && active[0] === z[0] ? 'active ' + z[3] : ''}"><i>${z[0]}</i><b>${esc(z[4])}</b></span>`).join('')}
    <span class="band-segment no-chase ${activeLabel === '不追价' ? 'active' : ''}"><i>不追价</i><b>${p.no_chase_price != null ? '&gt; ' + num(p.no_chase_price, d) : '—'}</b></span>
  </div><div class="band-caption"><span>静态位置</span><b>${esc(activeLabel)}</b></div>`;
}
...
function uiCardHtml(s, compact) {
  ...
  const flags = attention(s);
  const risk = wallTriggered(s.research) || ((s.research && s.research.core_validations) || []).some(v => v.status === '已恶化');
  const stale = q && q.is_stale;
  return `<article class="terminal-card ${compact ? 'compact-card' : ''} ${risk ? 'has-risk' : ''}" tabindex="0" data-sec-id="${s.id}" onclick="location.hash='#/s/${s.id}'">
    ...
    <div class="terminal-card-meta">${stale ? '<span class="stale-dot">●</span> 上次成功行情' : (q ? esc(quoteTimeHint(q)) : '尚未获取行情')}</div>
    ${uiPositionBand(s)}
    <div class="exec-focus ${exec ? '' : 'is-empty'}">
    ...
    <div class="next-action"><span>下一动作</span><b>${esc(plan.next_action || '—')}</b></div>
    ${risk ? `<div class="risk-line">${wallTriggered(s.research) ? '危墙已触发' : '验证项已恶化'}</div>` : (flags.length ? `<div class="attention-line">${esc(flags[0])}</div>` : '')}
    ${compact ? '' : uiQuickActions(s)}
  </article>`;
}

function renderTerminalHome(el) {
  const attentionSecs = S.secs.filter(s => attention(s).length > 0);
  const trackingSecs = S.secs.filter(s => !attention(s).length);
  const risks = uiRiskCount();
  ...
  el.innerHTML = `<div class="workspace-shell">
    <div class="workspace-hero">
      <div><div class="eyebrow">FOCUS / DENSE / FAST</div><h1>今日工作台</h1><p>${dateText} · 只看今天需要处理的变化</p></div>
      <div class="data-pulse"><span class="pulse-dot"></span><span>数据状态</span><b>${S.quoteError ? '行情有异常' : '已同步'}</b><small>${esc(S.lastSuccessAt || '等待首次刷新')}</small></div>
    </div>
    <div class="summary-grid">
      <div class="summary-card highlight"><span>需要处理</span><b>${attentionSecs.length}</b><small>今天优先动作</small></div>
      <div class="summary-card"><span>继续跟踪</span><b>${trackingSecs.length}</b><small>暂无新增变化</small></div>
      <div class="summary-card danger"><span>风险提示</span><b>${risks}</b><small>危墙 / 验证项</small></div>
      <div class="summary-card"><span>全部标的</span><b>${S.secs.length}</b><small>研究池总览</small></div>
    </div>
    <section class="work-section"><div class="section-heading"><div><div class="section-kicker">PRIORITY QUEUE</div><h2>需要处理</h2></div><span>${attentionSecs.length} 个标的</span></div>
      ${attentionSecs.length ? `<div class="terminal-grid">${attentionSecs.map(s => uiCardHtml(s, false)).join('')}</div>` : '<div class="empty-state"><span>✓</span><div><b>今天没有需要处理的变化</b><small>继续观察市场与研究事实</small></div></div>'}
    </section>
    <section class="work-section tracking-section"><div class="section-heading"><div><div class="section-kicker">WATCHLIST</div><h2>继续跟踪</h2></div><span>${trackingSecs.length} 个标的</span></div>
      ${trackingSecs.length ? `<div class="tracking-list">${trackingSecs.map(s => uiCardHtml(s, true)).join('')}</div>` : '<div class="empty-state"><div><b>暂无跟踪标的</b></div></div>'}
    </section>
  </div>`;
}
  ...
  return `<div class="drawer-mask" onclick="if(event.target===this)closeDrawer()"><aside class="detail-drawer" role="dialog" aria-label="标的详情">
    ...
    <div class="drawer-scroll">
      <div class="drawer-header"><div class="eyebrow">${esc(s.market || s.exchange)} · ${esc(s.currency)}</div><h2>${esc(s.name)}</h2><div class="drawer-code">${esc(s.code)} · ${esc(s.exchange)} ${statusBadge(s.status)}</div>
        <div class="drawer-quote"><div><b>${q ? curSym(s.currency) + ' ' + num(q.current, dec) : '暂无行情'}</b><span class="${q && q.change_pct >= 0 ? 'up' : 'down'}">${q ? (q.change_pct >= 0 ? '+' : '') + num(q.change_pct, 2) + '%' : '—'}</span></div></div>
      </div>
      ...
      <section class="drawer-section"><div class="drawer-section-title"><span class="section-kicker">03 · RESEARCH FACTS</span><button class="btn sm ghost" onclick="openResearchModal(${s.id})">查看 / 更新</button></div><h3>核心验证项 <small>${vals.length} 项 · ${validationSummary(r)}</small></h3>${vals.length ? `<details class="drawer-disclosure"><summary>展开验证项</summary><ul class="vlist">${vals.map(v => `<li><span class="tag ${v.status === '已恶化' ? 'worse' : v.status === '已验证' ? 'proven' : 'track'}">${esc(v.status)}</span><span>${esc(v.content)}</span></li>`).join('')}</ul></details>` : '<div class="drawer-empty">尚未录入核心验证项</div>'}</section>
      <section class="drawer-section"><div class="drawer-section-title"><span class="section-kicker">04 · WALLS</span>${wallCount ? '<span class="risk-pill">已触发 ' + wallCount + ' 项</span>' : '<span class="safe-pill">未触发</span>'}</div><h3>危墙</h3>${walls.length ? `<ul class="vlist wall-list">${walls.map(w => `<li class="${w.triggered ? 'wall-fired' : ''}"><span class="tag ${w.triggered ? 'fired' : 'safe'}">${w.triggered ? '已触发' : '未触发'}</span><span>${esc(w.content)}</span></li>`).join('')}</ul>` : '<div class="drawer-empty">尚未录入危墙条件</div>'}</section>
...
async function openDetailDrawer(id) {
  ...
  try { root.innerHTML = uiDrawerDetail(await api('/api/securities/' + id)); S.drawerId = Number(id); document.body.classList.add('drawer-open'); } catch (e) { root.innerHTML = ''; }
}
function closeDrawer() { const root = document.getElementById('drawer-root'); if (root) root.innerHTML = ''; document.body.classList.remove('drawer-open'); S.drawerId = null; if (routeInfo().page === 'detail') history.pushState({}, '', '#/home'); }
