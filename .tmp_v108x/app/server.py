...
def _import_cache_take(token, expected_kind):
    """校验并取出 preview cache 条目（不做失效操作；调用方决定何时失效）。

    拒绝条件（任一命中即拒绝）：
      - token 不是非空字符串 / 长度异常（客户端伪造的 base64 必然命中）
      - token 不在服务端 cache 中（未 preview / 已使用 / 已清理）
      - token 已过期（超过 TTL）
      - token kind 与当前接口不匹配
    """
    if not isinstance(token, str) or not token.strip():
        raise ImportTokenError('token 缺失或格式不合法：本接口只接受服务端 preview 返回的 token')
    tok = token.strip()
    now = time.time()
    with _IMPORT_PREVIEW_LOCK:
...
def _import_cache_invalidate(token):
    """commit 成功后立即让 token 失效（一次性）。"""
    with _IMPORT_PREVIEW_LOCK:
        _IMPORT_PREVIEW_CACHE.pop(token, None)
...
def _import_snapshot_drift(conn, snapshot):
    """对照 snapshot 重新读库，返回漂移描述列表（空列表 = 未漂移）。

    spec 第三节明确列出必须检测的 4 类漂移：
      - status 改变；
      - research 最新 version 改变；
      - trade_plan 最新 version 改变；
      - 原不存在的证券已经出现（以及反向：预览时存在、现在消失）。
    本函数严格只判定这 4 类 —— 不额外引入未在 spec 中列出的规则。

    注：snapshot 里保存的 execution_latest_id 本轮**不参与漂移判定**，
    仅作为 preview 快照的审计留痕（spec 第一节要求保存该字段）。
    """
    drifts = []
...
def preview_import_full(payload):
    ...
    try:
        ...
        snapshot = [
            _import_snapshot_for(conn, s['identity']['exchange'], s['identity']['code'])
            for s in validated
        ]
    finally:
        conn.close()
...
def commit_import_full(token, confirmed_status_changes=None):
    ...
    v1.0.8：
      ...
      - 成功后 token 立即失效（一次性）。
    """
    entry = _import_cache_take(token, IMPORT_FORMAT_FULL)
...
def preview_import_execution(payload):
    ...
    try:
        ...
        snapshot = [_import_snapshot_for(conn, ex, cd)]
...
def commit_import_execution(token):
    ...
    成功后 token 立即失效。
    """
    entry = _import_cache_take(token, IMPORT_FORMAT_EXEC_ONLY)
    validated = entry['validated']
    snapshot = entry['snapshot']
...
class Handler(BaseHTTPRequestHandler):
    server_version = 'Workbench/1.0.8'
...
def main():
    ap = argparse.ArgumentParser(description='A/H 投研交易工作台 v1.0.8')
    ...
    try:
        ...
        webbrowser.open(url)
        return
    print('A/H 投研交易工作台 v1.0.8 已启动: %s' % url)
