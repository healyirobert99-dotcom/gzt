...
function renderImportFullPreview(el) {
  ...
  el.innerHTML = `
    ...
    <div class="mfoot">
      <button type="button" class="btn ghost" onclick="goBackToPaste('full')">返回修改</button>
      <button class="btn primary" type="button" onclick="commitFull()" ${statusConfirmRequired ? 'disabled' : ''}>确认并写入</button>
    </div>
  `;
}
...
function renderImportExecPreview(el) {
  ...
  el.innerHTML = `
    ...
    <div class="mfoot">
      <button type="button" class="btn ghost" onclick="Import.step='paste';render();">返回修改</button>
      <button class="btn primary" type="button" onclick="commitExec()">确认并写入</button>
    </div>
  `;
}
